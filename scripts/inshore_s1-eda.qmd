...
#run the naming cleaning function to make sure all columns are named how we wnat
...
historical_metadata <- read_excel(
  glue("{data_path}/dt_wq_inshore_metadata.xlsx"),
...
for (i in 2:length(sheets)){
  ...
  assign(sheets[i], temp_sheet |> 
           mutate(across(c(3:ncol(temp_sheet)), as.numeric),
...
inshore_wq_all <- inshore_wq |> 
  ...
  summarise(Values = mean(Values, na.rm = TRUE)) |> ungroup() |>  #get daily mean
...
inshore_wq_all <- filter(inshore_wq_all, !Code %in% historical_metadata)
...
num_locs <- ifelse(script_fyear <= 2020, 7, 7)
...
stop("Test fail, currently the DT report should have 7 unique geographic areas, please cross check if 
...
num_codes <- ifelse(script_fyear <= 2020, 18, 18)
...
stop("Test fail, currently the DT report should have 18 unique codes(sites), please cross check if 
...
if (any("Val_Fail" %in% colnames(wqo_lor_check_p1))){
  ...
  wqo_lor_check_p2 <- wqo_lor_check_p1 |> 
    ...
    summarise(across(contains("Val"), ~ sum(.x)),
              KeepSite = case_when(Val_Fail < (if(exists("wqo_lor_check_p1$Val_Warn")){Val_Warn + Val_Pass} else {Val_Pass}) ~ "Keep", 
  ...
  if (!any(str_detect(wqo_lor_check_p2$KeepSite, "Rem"))){
    ...
    write_csv(inshore_wq_all, glue("{output_path}/spreadsheets/inshore_wq_all.csv"))
    ...
    removal_table <- semi_join(filter(inshore_wq_all, Fy == script_fyear), removal_table, by = join_by(Code, Indicator))
    ...
    write_csv(inshore_wq_all, glue("{output_path}/spreadsheets/inshore_wq_all_pre_site_removal.csv"))
    write_csv(inshore_wq_removed, glue("{output_path}/spreadsheets/inshore_wq_all_post_site_removal.csv"))
    
    #save the table that lists what was actually removed
    write_csv(removal_table, glue("{output_path}/spreadsheets/inshore_wq_all_sites_removed.csv"))
...
To make sure we don't miss anything we will go indicator by indicator:
 ...
 - Chla:
    + CB2 has one value greater than 40 with all other values around 5 or less. Possibly an error? However not considered an outlier
    + ANSWER: Value does line up with spikes in other indicators, no changes to value.
    + Geoffrey has tow outliers up in the 20-30s, although these seem like more reasonable values
    + ANSWER: No change to value
 - FRP:
 ...
 - PP:
    + Nothing major to note
 - Secchi:
    + BUR 1 has a secchi depth of 20m, which seems very high. Possibly this is meant to be 2? However this did happen mid-year, with low rainfall?
    + ANSWER: No obvious errors, no change to value.
 - TN:
    + Nothing major to note
 - TP:
    + Nothing major to note
 - TSS:
    + CB2 has 2 values greater than 110, this might be a bit high?
    + ANSWER: No obvious errors, no change to value.
 - Turbidity:
    + CB2 has one value greater than 110, this does align with the second TSS value noted above, possibly a correct value?
    + ANSWER: Possible the value (103) was meant to be 10.3 because every other values has one decimal, however there is no proof for this. Value has not been changed.
    + S2 has many many values above 200 or even 300. These do occur in series, however are likely unusually high.
    + ANSWER: Almost all of these values have been highlighted in the original spreadsheet using blue. There is no comment under S2, but comments under Beagle suggest the blue means outlier. This rows have been removed.
    

```{r}
#| label: action EDA findings

print("For the year of 2024 several values were investigated for notably high/low values (using reasoning) as well as values that were flagged as statistical outliers. Several values have been removed from the S2 logger based on flags provided by the supplier. No other values were changed.")

print("HISTORICAL: For the year of 2023, there were several Turbidity values dectected by the S2 logger that were more than 1000 times greater than the preceeding and following recorded values. These values (5 total values) were removed from the dataset.")
...
if (exists("inshore_wq_removed")) {# a removed and non-removed set exists, save both
  ...
  write_csv(inshore_wq_all, 
            glue("{data_path}/{script_fyear-1}-{script_fyear}_inshore_wq_all_sites_removed.csv"))
  ...
  write_csv(inshore_wq_pre_removal, 
            glue("{data_path}/{script_fyear-1}-{script_fyear}_inshore_wq_all_pre_removal.csv"))

} else {#otherwise just save the one
  
  write_csv(inshore_wq_all, 
            glue("{data_path}/{script_fyear-1}-{script_fyear}_inshore_wq_all.csv"))
