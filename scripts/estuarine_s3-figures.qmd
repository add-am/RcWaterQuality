...
estuarine_wq_target <- estuarine_wq_all |> filter(Fy <= script_fyear)
...
boxplot_data <- estuarine_wq_target |> filter(Fy == script_fyear)
...
for (i in unique(boxplot_data$Indicator)){#for each of the indicators
  ...
  if (str_detect(i, "Do")){#if the indicator is dissolved oxygen (high or low)
    ...
    target_data <- boxplot_data |> filter(Indicator == c("HighDo", "LowDo"))
  ...
  if(str_detect(i, "Do")){#if the indicator is dissolved oxygen (high or low)
    ...
    box_plot <- box_plot +
      geom_point(target_data |> filter(Indicator == "LowDo"), 
...
names(lineplot_colours) <- levels(unique(estuarine_wq_target$WatercourseOrGeographicArea))
...
for (i in unique(estuarine_wq_target$Indicator)){#for each indicator
  ...
  #check if there is more than one WQO
  checking_wqo <- target_data |> 
    select(SubBasinOrSubZone, Wqo) |> 
    unique()

  if (length(unique(checking_wqo$SubBasinOrSubZone)) != length(checking_wqo$Wqo)){
    mul_wq <- TRUE 
  } else {
    mul_wq <- FALSE
  }
  ...
  if (mul_wq){#if there is more than one wqo, split them up

    labels <- target_data |> 
      group_by(WatercourseOrGeographicArea) |> 
      slice(n() %/% 2) |> 
      ungroup()

    line_plot <- line_plot +
      geom_line(mapping = aes(x = Date, y = Wqo, group = WatercourseOrGeographicArea), color = "black") +
      geom_text(data = labels, aes(x = Date, y = Wqo, label = WatercourseOrGeographicArea), vjust = -0.5, hjust = 0.5)

  } else {#otherwise they can remain together
