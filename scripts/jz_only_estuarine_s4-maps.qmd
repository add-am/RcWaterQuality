...
sites <- read_excel(path = glue("{data_path}/jz_wq_estuarine_metadata.xlsx"), sheet = "Current_Sites") |> 
  name_cleaning() |> 
...
for (i in target_vect){
  ...
  focus_est <-  dt_est_only |>  filter(BasinOrZone == i) |> rename("Sub Basin: All" = SubBasinOrSubZone)
  ...
  map <- tm_shape(qld) +
    ...
    tm_text("Code", 
            size = 0.9,
            options = opt_tm_text(shadow = T, point.label = T, point.label.gap = 1)) +
    tm_layout(legend.bg.color = "white", 
              legend.position = c("left", "bottom"),
              asp = 1)
  
  #and save
  tmap_save(map, glue("{output_path}/{i}_Estuarine.png"), insets_tm = inset_map, insets_vp = inset_viewport)
...
for (i in target_vect){
  ...
  map <- tm_shape(qld) +
    ...
    tm_symbols(size = 0.6, 
               ...
               lwd = 2, 
               shape = 21) +
    #tm_text("Code",  #hidden as this is the deidentified maps
    #        size = 0.9,
    #        options = opt_tm_text(shadow = T, point.label = T, point.label.gap = 1)) +
    tm_layout(legend.bg.color = "white", 
              legend.position = c("left", "bottom"),
  ...
  #save map
  tmap_save(map, glue("{output_path}/{i}_de-identified_Estuarine.png"), insets_tm = inset_map, insets_vp = inset_viewport)
