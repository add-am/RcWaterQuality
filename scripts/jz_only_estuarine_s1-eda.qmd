---
title: "Jet Zero Estuarine Water Quality Analysis"
...
for (i in 1:length(target_sheets)){
  ...
  temp_sheet <- read_excel(glue("{data_path}jz_wq_estuarine_metadata.xlsx"),
...
sheets <- excel_sheets(glue("{data_path}jz_wq_estuarine_data_master.xlsx"))

#get the total number of sheets excluding the first one (which is the README sheet)
for (i in 2:length(sheets)){
  
  #read in the target sheet
  temp_sheet <- read_excel(glue("{data_path}jz_wq_estuarine_data_master.xlsx"), 
...
#estuarine_wq <- bind_rows(DES, TCC_Wastewater, TCC_Other, CLMP) 
...
We will check the number of locations found in the dataset matches what we expect to find (which for estuarine is 16 unique watercourses and 24 unique codes (sites)).
...
if (nrow(location_check) == 2 & !any(is.na(location_check))){
...
if (nrow(code_check) == 4 & !any(is.na(code_check))){
...
stop("Test fail, currently the DT report should have 3 unique codes(sites), please cross check if 
...
if (any("Val_Fail" %in% colnames(wqo_lor_check_p1))){
  ...
  if (!any(str_detect(wqo_lor_check_p2$Keep_Site, "Rem"))){
    ...
    write_csv(wq_all, glue("{output_path}/spreadsheets/estuarine_wq_all.csv"))
    ...
    estuarine_wq_removed <- suppressMessages(anti_join(wq_all, removal_table))
      
    #save the before and after site removal tables
    write_csv(wq_all, glue("{output_path}/spreadsheets/estuarine_wq_all_pre_site_removal.csv"))
...
The code chunk above has automatically removed the offending indicators from the offending sites. Now, in the code chunk below, we will overwrite the main estuarine dataset with the dataset we just created that has some values removed.
...
if (exists("estuarine_wq_removed")){
  ...
  estuarine_wq_pre_removal <- wq_all
  
  wq_all <- estuarine_wq_removed}
...
estuarine_wq_cy <- wq_all |> filter(Fy == current_fyear)
