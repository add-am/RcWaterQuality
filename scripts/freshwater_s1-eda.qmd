...
location_check <- freshwater_wq_all |>
  ...
  group_by(Region, Environment, BasinOrZone, SubBasinOrSubZone, Watercourse) |> 
  summarize(.groups = "drop")
...
dry_tropics <- n3_region |> 
  filter(Environment == "Freshwater", Region == "Dry Tropics")
...
if (file.exists(path) == T){ #if it exists, read it in
  ...
  sub_basin_rain <- pivot_longer(sub_basin_rain, cols = 6:ncol(sub_basin_rain),
    ...
    mutate(Date = ymd_hms(paste(layer_date, "23:00:00")),
           SubBasinOrSubZone = case_when(str_detect(SubBasinOrSubZone, "Lower") ~ "Lower Ross River",
                                 str_detect(SubBasinOrSubZone, "Upper") ~ "Upper Ross River",
                                 str_detect(SubBasinOrSubZone, "Palum") ~ "Paluma Dam",
                                 T ~ Sub_Basin)) |> 
    select(SubBasinOrSubZone, Rainfall, Date)
...
for (i in unique(freshwater_wq_all$Indicator)){#for each indicator
  for (j in unique(freshwater_wq_all$SubBasinOrSubZone)){#and each sub basin
    ...
    freshwater_targeted <- freshwater_wq_all |> filter(Indicator == i, SubBasinOrSubZone == j)
    rainfall_targeted <- sub_basin_rain |> filter(SubBasinOrSubZone == j, Date >= min(freshwater_targeted$Date))
