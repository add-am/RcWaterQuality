...
if (length(unique(unit_check$Indicator)) == n_distinct(unit_check)){
  ...
  #save csv
  write_csv(unit_check, glue("{output_path}/spreadsheets/jz_unit_check.csv"))
...
if (nrow(location_check) == 1 & !any(is.na(location_check))){
  ...
  #save csv
  write_csv(location_check, glue("{output_path}/spreadsheets/jz_location_check.csv"))

stop("Test fail, currently the DT report should have 1 unique watercourses, please cross check if 
...
if (nrow(code_check) == 2 & !any(is.na(code_check))){
  ...
  #save csv
  write_csv(code_check, glue("{output_path}/spreadsheets/jz_code_check.csv"))
...
if (!any(value_lor_check$ValueHalved > value_lor_check$ValueUnchanged)){
  ...
  write_csv(value_lor_check, glue("{output_path}/spreadsheets/jz_value_lor_check.csv"))
...
if (any("Val_Fail" %in% colnames(wqo_lor_check_p1))){
  ...
  write_csv(wqo_lor_check_p2, glue("{output_path}/spreadsheets/jz_wqo_lor_check.csv"))
  ...
  if (!any(str_detect(wqo_lor_check_p2$Keep_Site, "Rem"))){
    ...
    write_csv(wq_all, glue("{output_path}/spreadsheets/jz_freshwater_wq_all.csv"))
    ...
    removal_table <- semi_join(filter(wq_all, Fy == script_fyear), removal_table, by = join_by(Code, Indicator))
    ...
    write_csv(wq_all, glue("{output_path}/spreadsheets/jz_freshwater_wq_all_pre_site_removal.csv"))
    write_csv(freshwater_wq_removed, glue("{output_path}/jz_spreadsheets/freshwater_wq_all_post_site_removal.csv"))
    
    #save the table that lists what was actually removed
    write_csv(removal_table, glue("{output_path}/spreadsheets/jz_freshwater_wq_all_sites_removed.csv"))
...
freshwater_wq_cy <- wq_all |> filter(Fy == script_fyear)
...
for (i in unique(freshwater_wq_cy$Indicator)){#for each indicator
  ...
  #save to file
  ggsave(glue("{output_path}/plots/jz_{i}_histogram.png"))
...
for (i in unique(freshwater_wq_cy$Indicator)){#for each indicator
  ...
  #save to file
  ggsave(glue("{output_path}/plots/jz_{i}_boxplot.png"))
...
for (i in unique(freshwater_wq_cy$Indicator)){#for each indicator
  ...
  #save to file
  ggsave(glue("{output_path}/plots/jz_{i}_lineplot.png"))
...
if (exists("freshwater_wq_removed")) {# a removed and non-removed set exists, save both
  ...
  write_csv(wq_all, 
            glue("{data_path}/jz_{script_fyear-1}-{script_fyear}_freshwater_wq_all_sites_removed.csv"))
  ...
  write_csv(freshwater_wq_pre_removal, 
            glue("{data_path}/jz_{script_fyear-1}-{script_fyear}_freshwater_wq_all_pre_removal.csv"))

} else {#otherwise just save the one
  
  write_csv(wq_all, 
            glue("{data_path}/jz_{script_fyear-1}-{script_fyear}_freshwater_wq_all.csv"))
