---
title: "Jet Zero Estuarine Water Quality Exploratory Data Analysis"
...
estuarine_wq_all <- estuarine_wq_all |> 
  mutate(across(c(BasinOrZone, SubBasinOrSubZone, WatercourseOrGeographicArea), factor),
         ...
         WatercourseOrGeographicArea = fct_relevel(WatercourseOrGeographicArea, "Bohle River", "Louisa Creek", "Ross Creek", "Ross River", 
                                   ...
                                   "Crystal Creek", "Outside Townsville State Development Area", "Inside Townsville State Development Area")) 
...
save_n3_table(
  ...
  file_name = glue("{output_path}/est_appendix_sub_basin_scores"), #pick a name for your output file
...
est_wq_sub_basin_weighted <- est_wq_all_scores |> 
  mutate(
    across(c("Din", "Frp", "HighDo", "LowDo", "Tp", "Turbidity", "Nutrients", "PhysChem", "OverallWq"),
...
est_wq_basin_weighted <- est_wq_sub_basin_weighted |> 
  ...
  mutate(
    across(
      c("SubBasinArea","DinWeighted", "FrpWeighted", "HighDoWeighted", "LowDoWeighted", 
...
save_n3_table(
  ...
  file_name = glue("{output_path}/est_nutrients"), #pick a name for your output file
...
#run the "save as n3 table" function
save_n3_table(
  df = PhysChem_table, #select your data frame
  file_name = glue("{output_path}/est_phys_chem"), #pick a name for your output file
  target_columns = 4:11, #specify which columns to format
  target_rows = 1:nrow(PhysChem_table), #specify which rows to format
  scheme = "Report Card", #define the formatting scheme
  include_letter = FALSE #decide if you want to include letter grades
)
