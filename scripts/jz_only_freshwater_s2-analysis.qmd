...
pacman::p_load(tidyverse, glue, here, janitor, openxlsx2, reactable, pak)
...
freshwater_wq_all <- freshwater_wq_all |> 
  mutate(across(c(BasinOrZone, SubBasinOrSubZone, WatercourseOrGeographicArea), factor),
         ...
         WatercourseOrGeographicArea = fct_relevel(WatercourseOrGeographicArea, "Ross Lake", "Aplins Weir", "Gleesons Weir",
                                   ...
                                   "Paluma Lake", "Outside Townsville State Development Area", "Inside Townsville State Development Area")) 
...
#save data
save_n3_table(
  df = freshwater_summary_stats,
  file_name = glue("{output_path}/summary_statistics"),
  target_columns = 5,
  target_rows = 1:nrow(freshwater_summary_stats),
...
#clean the names up
fw_wq_annual_wide <- name_cleaning(fw_wq_annual_wide)
...
group_by(Region, BasinOrZone, SubBasinOrSubZone, SubBasinArea, SubBasinProportionOfBasin) |> 
  summarise(across(c(Din, HighDo, LowDo, Turbidity, Tp, Nutrients, PhysChem, OverallWq), mean, na.rm = T))
...
save_n3_table(
  df = sub_basin_scores_save, #select your data frame
  file_name = glue("{output_path}/fw_appendix_sub_basin_scores"), #pick a name for your output file
  target_columns = 2:4, #specify which columns to format
  target_rows = 1:nrow(sub_basin_scores_save), #specify which rows to format
  scheme = "Report Card", #define the formatting scheme
  include_letter = FALSE #decide if you want to include letter grades
)
...
fw_wq_sub_basin_weighted <- fw_wq_all_scores |> 
  mutate(
    across(c("Din", "HighDo", "LowDo", "Tp", "Turbidity", "Nutrients", "PhysChem", "OverallWq"),
    \(x) x*SubBasinProportionOfBasin, 
    .names = "{.col}Weighted"))
...
fw_wq_basin_weighted <- fw_wq_sub_basin_weighted |> 
  ...
  mutate(
    across(
      c("SubBasinArea","DinWeighted", "HighDoWeighted", "LowDoWeighted", 
      "TpWeighted", "TurbidityWeighted", "NutrientsWeighted", "PhysChemWeighted", "OverallWqWeighted"),
      \(x) if_else(is.na(SubBasinOrSubZone), sum(x, na.rm = TRUE), x))) |> ungroup()
...
fw_wq_all_scores_grades_raw <- fw_wq_all_weighted |> 
  mutate(
    across(9:length(fw_wq_all_weighted), 
    \(x) score_to_grade(x),
    .names = "{.col}_Grade"))
...
write_csv(fw_wq_all_scores_grades_raw, glue("{output_path}/fw_wq_all_scores_grades_raw.csv"))

#save entire rounded dataset
write_csv(fw_wq_all_scores_grades_floor, glue("{output_path}/fw_wq_all_scores_grades_floor.csv"))
...
#run the "save as n3 table" function
save_n3_table(
  df = nutrients_table, #select your data frame
  file_name = glue("{output_path}/fw_nutrients"), #pick a name for your output file
  target_columns = 4:10, #specify which columns to format
  target_rows = 1:nrow(nutrients_table), #specify which rows to format
  scheme = "Report Card", #define the formatting scheme
  include_letter = FALSE #decide if you want to include letter grades
)
...
#run the "save as n3 table" function
save_n3_table(
  df = phys_chem_table, #select your data frame
  file_name = glue("{output_path}/fw_phys_chem"), #pick a name for your output file
  target_columns = 4:11, #specify which columns to format
  target_rows = 1:nrow(phys_chem_table), #specify which rows to format
  scheme = "Report Card", #define the formatting scheme
  include_letter = FALSE #decide if you want to include letter grades
)
