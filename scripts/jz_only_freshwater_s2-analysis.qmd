...
data_path <- "C:/Users/adams/GitHub/RcWaterQuality/data/freshwater/processed/jz_2024-2025_freshwater_wq_all"
...
fw_wq_annual_scored <- value_to_score(
  df = fw_wq_annual,
...
fw_wq_annual_scored <- fw_wq_annual_scored |> 
  select(
...
fw_wq_annual_wide <- fw_wq_annual_scored |> 
...
save_n3_table(
  df = sub_basin_scores_save, 
  file_name = glue("{output_path}/fw_appendix_sub_basin_scores"), 
  target_columns = 2:4, 
  target_rows = 1:nrow(sub_basin_scores_save), 
  scheme = "Report Card"
)
...
fw_wq_all_scores_grades_raw <- score_to_grade(fw_wq_all_weighted, 7:length(fw_wq_all_weighted))
...
physchem_table <- nut_phys_chem_prep |> 
  select(BasinOrZone, SubBasinOrSubZone, WatercourseOrGeographicArea, HighDo, LowDo, Turbidity, 
  PhysChem, "Weighting(%)", "Area(km2)", SubBasinPhysChemWeighted, PhysChemWeighted) |> 
  arrange(BasinOrZone, SubBasinOrSubZone, WatercourseOrGeographicArea)

save_n3_table(
  nutrients_table,
  glue("{output_path}/fw_nutrients"),
  c(4:10),
